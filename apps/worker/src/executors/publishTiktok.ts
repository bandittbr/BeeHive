// Executor de publicação no TikTok (Content Posting API).
// Fluxo: refresh do access_token → init de upload (FILE_UPLOAD) → PUT dos bytes →
// poll do status. Credenciais por conta: clientKey, clientSecret, refreshToken.
// Obs.: apps não auditados só publicam como SELF_ONLY (privado). Após a auditoria
// da Content Posting API, use PUBLIC_TO_EVERYONE.
...
async function refreshAccessToken(clientKey: string, clientSecret: string, refreshToken: string): Promise<string> {
  ...
  if (!res.ok || !json.access_token) {
    throw new Error(`OAuth TikTok falhou: ${json.error_description || json.error || res.status}`);
  }
...
async function apiJson(urlPath: string, accessToken: string, body: unknown): Promise<any> {
  ...
  if (!res.ok || (err && err.code && err.code !== 'ok')) {
    throw new Error(err?.message || `TikTok HTTP ${res.status}`);
  }
  return json;
}
...
export async function runPublishTiktok(req: JobRequest, onChunk: Chunk): Promise<{ result: unknown }> {
  ...
  if (!file || !clientKey || !clientSecret || !refreshToken) {
    throw new Error('tiktok: file, clientKey, clientSecret e refreshToken são obrigatórios');
  }
  const allowed = ['SELF_ONLY', 'PUBLIC_TO_EVERYONE', 'MUTUAL_FOLLOW_FRIENDS', 'FOLLOWER_OF_CREATOR'];
  const privacyLevel = allowed.includes(String(p.privacyLevel)) ? String(p.privacyLevel) : 'SELF_ONLY';
  const title = String(p.title ?? '').slice(0, 2200);

  const dir = resolveInWorkspace(req.cwd ?? '.');
  const abs = path.join(dir, file);
  if (!fs.existsSync(abs)) throw new Error(`tiktok: arquivo não encontrado: ${file}`);
  ...
  if (!put.ok) {
    ...
    throw new Error(`tiktok: upload falhou (${put.status}): ${t.slice(0, 200)}`);
  }
  ...
  if (finalStatus !== 'PUBLISH_COMPLETE') throw new Error('tiktok: timeout no processamento');

  const url = postId ? `https://www.tiktok.com/video/${postId}` : undefined;
  onChunk('stdout', `✓ Publicado no TikTok${url ? `: ${url}` : ` (${privacyLevel})`}\n`);
  return { result: { platform: 'tiktok', id: postId || publishId, url, privacyLevel } };
}
