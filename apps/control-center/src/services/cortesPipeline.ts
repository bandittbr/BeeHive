...
// IA escolhe os melhores momentos → cortar em vertical (worker) → links dos clipes.
...
export async function generateCortes(input: {
  ...
  const fetchRes = await runWorkerJob({ type: 'ytFetch', payload: { url } }, { timeoutMs: 540000 });
  ...
  prog('Analisando e escolhendo os melhores momentos...');
  const segPrompt = `Você recebe a transcrição SRT de um vídeo. Escolha os ${count} MELHORES momentos para virarem cortes virais de 30-60s (gancho forte, ideia completa).
Responda SOMENTE com JSON:
{ "segments": [ { "start": "HH:MM:SS", "end": "HH:MM:SS", "title": "legenda curta e chamativa" } ] }
Use os timestamps reais do SRT. No máximo ${count} segmentos.

SRT:
${String(data.transcript).slice(0, 40000)}`;

  let segments: any[] = [];
  try {
    const raw = await askBeeHive(segPrompt);
    const parsed = extractJson(raw);
    segments = Array.isArray(parsed?.segments) ? parsed.segments.slice(0, count) : [];
  } catch { /* vazio abaixo */ }
  if (segments.length === 0) return { clips: [], error: 'Não consegui identificar bons momentos na transcrição.' };

  // 3) cortar em vertical no worker
  prog(`Cortando ${segments.length} trecho(s) em vertical...`);
  const clipRes = await runWorkerJob(
    { type: 'clip', payload: { input: data.video, segments, vertical: true } },
    { timeoutMs: 540000 },
  );
