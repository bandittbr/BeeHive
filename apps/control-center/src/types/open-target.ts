...
export function isCollectibleArtifactTarget(target: OpenTarget) {
  const PREVIEWS = new Set<OpenTargetPreview>(["markdown", "sheet", "slides", "document", "image", "pdf", "html"])
  return target.kind === "file" && target.exists === true && PREVIEWS.has(target.preview)
}
...
const WRITE_TOOL_NAMES = new Set([
  "apply_patch", "edit", "edit_file", "multi_edit", "multiedit",
  "patch", "str_replace_editor", "write", "write_file",
])
...
export function deriveOpenTargets(messages: MessageLike[]): OpenTarget[] {
    ...
    for (const part of message.parts) {
      if (part.type === "text" && typeof part.text === "string") {
        scanText(targets, part.text, message.role === "assistant" ? 65 : 40, "message", {
          includeFiles: message.role === "assistant" && shouldScanAssistantFileMentions(part.text),
        })
        continue
      }
      ...
      if (isWrite) {
        addFileValues(targets, collectFileMetadataValues(part.input), 95, "write tool metadata")
