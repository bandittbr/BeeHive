...
const PROJECTS: Project[] = [
  ...
  },
  {
    ...
    agents: [
      ...
      { id: 'c3', name: 'Video Agent', status: 'waiting', task: 'Na fila de renderização', color: '#8b5cf6' },
      { id: 'c4', name: 'Analytics Agent', status: 'idle', task: 'Ocioso', color: '#6366f1' },
    ],
    workflows: [
      { id: 'z1', name: 'Marketing Diário', status: 'running', progress: 64, project: 'Marketing' },
      { id: 'z2', name: 'Publicação Instagram', status: 'scheduled', progress: 0, project: 'Marketing' },
  ...
  {
    id: '4', name: 'Cliente X', icon: '💼', description: 'Consultoria', status: 'paused',
    agents: [
...
const ALL_MISSIONS: Mission[] = [
  { id: 'm1', name: 'Marketing Diário', progress: 64, status: 'running', agents: ['Marketing Agent', 'Content Agent'], project: 'Marketing' },
  { id: 'm2', name: 'Pesquisa Empresa X', progress: 38, status: 'running', agents: ['Research Agent'], project: 'Marketing' },
  { id: 'm3', name: 'Publicação YouTube', progress: 100, status: 'completed', agents: ['Video Agent'], project: 'Marketing' },
...
const ALL_EVENTS: Event[] = [
  { id: 'e1', type: 'success', text: 'Publicação YouTube concluída', time: '2 min', project: 'Marketing' },
  { id: 'e2', type: 'success', text: 'Workflow "Marketing Diário" atualizado', time: '15 min', project: 'Marketing' },
  { id: 'e3', type: 'warning', text: 'OpenRouter com latência alta', time: '30 min', project: 'BeeHive' },
  { id: 'e4', type: 'success', text: 'Browser scraping finalizado', time: '1h', project: 'BeeHive' },
  { id: 'e5', type: 'error', text: 'Trade BTC — conexão perdida', time: '2h', project: 'TradeAI' },
    ...
    <div className="app">
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <button className="nav-item" title="Novo Projeto"><Plus size={18} strokeWidth={1.5} /></button>
        </nav>

        <div className="sidebar-footer">
          <div className="sidebar-divider" />
          <button className="nav-item" title="Settings"><Settings size={18} strokeWidth={1.5} /></button>
        </div>
      </aside>
    ...
    <div className="mission-control">
      ...
      <div className="mc-hero">
        <div>
          ...
          <p>{runningAgents.length} agentes trabalhando · {runningMissions.length} missões ativas · {totalQueue} tarefas na fila</p>
        </div>
      </div>
      ...
      <div className="mc-grid">
        ...
        <div className="mc-panel mc-missions">
          <div className="mc-panel-header">
            <h2><Target size={16} /> Missões em Execução</h2>
        ...
        <div className="mc-panel mc-queue">
          ...
          <div className="queue-content">
            ...
            <div className="queue-breakdown">
              ...
              <span><CircleDot size={10} /> 5 automações</span>
            ...
            {PROJECTS.map(p => (
              <button key={p.id} className="quick-project" onClick={() => onOpenProject(p)}>
                ...
                <div className="qp-info">
                  ...
                  <span className="qp-meta">{p.agents.length} agents · {p.workflows.length} workflows</span>
                </div>
...
function ProjectChat({ project }: { project: Project }) {
  ...
  const handleSend = () => {
    ...
    setMessages(prev => [...prev, { id: String(Date.now()), role: 'user', content: input, time: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) }]);
    setInput('');
    setTimeout(() => {
  ...
  return (
    <div className="project-settings">
      ...
      <div className="settings-form">
        <div className="form-group"><label>Nome</label><input type="text" defaultValue={project.name} /></div>
        <div className="form-group"><label>Descrição</label><textarea defaultValue={project.description} rows={3} /></div>
