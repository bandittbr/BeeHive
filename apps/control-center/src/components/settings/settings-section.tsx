...
export function SettingsInset({ children, className }: SettingsInsetProps) {
  return (
    <div className={cn("border border-border rounded-2xl p-4", className)}>
      {children}
    </div>
  );
}
...
export interface SettingsNoticeProps extends SettingsStackProps {
  tone?: "neutral" | "error";
}
...
export function SettingsNotice({
  ...
  return (
    <div
      className={cn(
        "rounded-xl border border-border bg-muted px-3 py-2 text-xs text-muted-foreground",
        tone === "error" && "border-red-500/30 bg-red-50 text-red-600",
        className,
      )}
    >
      {children}
...
export type SectionItemHeaderProps = SettingsStackProps;
...
export function SettingsSectionHeaderTitle({ children, className }: SettingsItemHeaderTitleProps) {
  return (
    <div className={cn("flex items-center gap-2 text-base font-medium text-foreground", className)}>
      {children}
    </div>
  );
}
