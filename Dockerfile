FROM node:22-slim

WORKDIR /app

RUN apt-get update && apt-get install -y --no-install-recommends git ffmpeg python3 python3-pip curl file \
  && rm -rf /var/lib/apt/lists/* \
  && pip3 install --no-cache-dir --break-system-packages yt-dlp -r AI-Youtube-Shorts-Generator/requirements.txt

RUN npm install -g pnpm@9

COPY pnpm-workspace.yaml package.json pnpm-lock.yaml tsconfig.base.json ./
COPY apps/worker/package.json apps/worker/
COPY packages/shared/package.json packages/shared/
COPY packages/sdk/package.json packages/sdk/
COPY plugins/*/package.json plugins/

RUN pnpm install --no-frozen-lockfile && \
  mkdir -p /app/node_modules/@beehive && \
  ln -sf /app/packages/sdk /app/node_modules/@beehive/sdk && \
  ln -sf /app/packages/shared /app/node_modules/@beehive/shared

# Chromium do Playwright (usado pela etapa "browser" do orquestrador — testar/screenshot de páginas geradas)
RUN pnpm --filter @beehive/worker exec playwright install --with-deps chromium

COPY . .

RUN pnpm --filter @beehive/worker run build || true

EXPOSE 3000

CMD ["pnpm", "--filter", "@beehive/worker", "run", "start"]